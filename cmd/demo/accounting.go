...
func (ds *DemoState) BuildBalanceSheetHTML() string {
	...
	s.WriteString(fmt.Sprintf(`<tr><td>Loans to Customers</td><td class="has-text-right">%s</td></tr>`, fmtMoney(totalLoans)))
	s.WriteString(fmt.Sprintf(`<tr><td>Cash &amp; Reserves</td><td class="has-text-right">%s</td></tr>`, fmtMoney(cash)))
	s.WriteString(`<tr><td colspan="2"><hr class="my-1"></td></tr>`)
	s.WriteString(fmt.Sprintf(`<tr><td class="has-text-weight-bold">Total Assets</td><td class="has-text-right has-text-weight-bold">%s</td></tr>`, fmtMoney(totalAssets)))
	...
	s.WriteString(fmt.Sprintf(`<tr><td>Retained Earnings</td><td class="has-text-right"%s>%s</td></tr>`, eqClass, fmtMoney(equity)))
	s.WriteString(`<tr><td colspan="2"><hr class="my-1"></td></tr>`)
