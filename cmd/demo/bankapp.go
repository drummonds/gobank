...
func (ds *DemoState) buildAppLoginHTML() string {
	...
	var s strings.Builder
	s.WriteString(phoneHeader("Model Bank", "Select your account"))
	...
	if len(customers) == 0 {
		...
		// Show up to 50 customers as cards
		limit := len(customers)
		if limit > 50 {
			limit = 50
		}
  ...
  <div style="background:#f5f5f5;border-radius:10px;padding:12px 16px;display:flex;align-items:center">
    ...
    <div><strong>%s</strong><br><span style="font-size:0.75rem;color:#7a7a7a">%s</span></div>
  </div>
...
func (ds *DemoState) buildAppBalanceHTML(custID string) string {
  ...
  <div style="display:flex;justify-content:space-between;font-size:0.75rem;opacity:0.85;margin-top:8px">
	...
	for _, a := range resp.Accounts {
		...
		fmt.Fprintf(&s, `<div style="background:#fafafa;border-radius:10px;padding:14px 16px;margin-bottom:8px;border-left:4px solid %s">
  <div style="display:flex;justify-content:space-between;align-items:center">
    <div><span>%s</span> <strong>%s</strong><br><span style="font-size:0.75rem;color:#7a7a7a">%.2f%% APR</span></div>
    <div style="text-align:right"><strong>%s</strong><br><span style="font-size:0.7rem;color:#7a7a7a">Interest: %s</span></div>
  </div>
</div>`, familyColor, familyIcon, a.ProductName, a.Rate*100, fmtMoney(a.Balance), fmtMoney(a.Interest))
	}
